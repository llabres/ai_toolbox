...
def load_config(args, eval_only=False): 
    ...
    config['model_checkpoint'] = os.path.join('/gpfs/projects/ehpc44/ai_toolbox/models', args.model.replace('-', '').replace('base', '').replace('large', ''), f"{args.model.lower()}")
...
def build_dataset(config, split):
    ...
    elif config['dataset'] == 'PFL-DocVQA':
        from my_datasets.pfl_docvqa import build_pfl_docvqa
        data_dir = os.path.join(config['data_dir'], 'PFL-DocVQA')
        dataset = build_pfl_docvqa(data_dir, split, config)
        dataset = dataset.shuffle(buffer_size=1000, seed=random.randint(0, 1000))

    elif config['dataset'] == 'OCR-IDL':
        from my_datasets.ocr_idl import build_ocr_idl
        data_dir = os.path.join(config['data_dir'], 'OCR-IDL')
        dataset = build_ocr_idl(data_dir, split)
        dataset = dataset.shuffle(buffer_size=1000, seed=random.randint(0, 1000))
...
def build_optimizer(config, model):
    ...
    scheduler_specific_kwargs = {'num_cycles': config['n_epochs'],'last_epoch': config['current_epoch']-1} if config['scheduler_name'] == 'cosine_with_restarts' else None
